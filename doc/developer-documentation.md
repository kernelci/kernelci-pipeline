...
### Example to enable a new kselftest

The kselftests (found in `<linux_kernel>/tools/testing/selftests`) are enabled suite by suite. Most parameters are already set for other kselftests. Those must be reused while enabling new kselftests which make enabling new tests pretty straight forward. The `dt` kselftest suite can be enabled by:
...
```
2. Create a `scheduler` entry for `kselftest-dt` job. Lets schedule this suite in `lava-collabora` lab at `kbuild-gcc-12-arm64` build completion event. The entry by using already defined anchors would be:
```
  - job: kselftest-dt
