...
class Metrics:
    ...
    def update(self):
        with self.lock:
            ...
            self.metrics["executor_threads_active"] = (
...
def _get_telemetry_emitter():
    ...
    if _telemetry_emitter is None:
        ...
        if api_config_name:
            ...
            _telemetry_emitter = TelemetryEmitter(api, "lava-callback")
...
def async_job_submit(api_helper, node_id, job_callback):
    ...
    try:
        ...
        if not job_node:
            ...
            return
        logger.debug(
            f"Processing node {node_id}: {job_node.get('name', 'unknown')}"
        )
        ...
        if log_parser:
            ...
            callback_json_url = _upload_callback_data(
                callback_data, job_node, storage
            )
        ...
        for name, state in upload_result.items():
            if name.startswith("artifact-upload:") and state == "pass":
                ...
                if len(artifact) != 3:
                    logger.warn(
                        f"Failed to extract artifact name and URL from {name}"
                    )
                    continue
...
def _emit_test_results(emitter, child_nodes, common, suite=None):
    ...
    for child in child_nodes:
        ...
        if kind == "test":
            ...
            _emit_test_results(emitter, sub_children, common, suite=name)
...
async def jobretry(
    data: JobRetry, request: Request, Authorization: str = Header(None)
):
    """
...
async def checkout(
    data: ManualCheckout, request: Request, Authorization: str = Header(None)
):
    """
    ...
    logger.info(
        f"User {email} is checking out {data.nodeid} at custom commit {data.commit}"
    )
    api_config_name = SETTINGS.get("DEFAULT", {}).get("api_config")
    if not api_config_name:
        item["message"] = "No default API name set"
...
async def patchset(
    data: PatchSet, request: Request, Authorization: str = Header(None)
):
    """
