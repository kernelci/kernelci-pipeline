...
in {runtime}",
    ...
    def _total_size(self, batchcheckouts, batchbuilds, batchtests, batchissues, batchincidents):
        """
        Watch out for total size of the batched data, as google cloud pubsub have
        limit of 10MB per message
        """
        size_bytes = 0
        for data in [batchcheckouts, batchbuilds, batchtests, batchissues, batchincidents]:
    ...
    def _run(self, context):
        ...
        while True:
            is_hierarchy = False
            subsize = self._total_size(batchcheckouts, batchbuilds, batchtests, batchissues, batchincidents)
            if subsize > 5000000:
                self.log.warning(f"Total size of batched data is {subsize} bytes")
            if not nodes or len(nodes) == 0 or subsize > 5000000:
