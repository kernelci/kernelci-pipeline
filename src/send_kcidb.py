...
the test: {sub_path}")
    ...
    def _get_parent_build_node(self, node):
        ...
        if parent_id in self._nodecache:
            node = self._nodecache[parent_id]
            self.log.debug(f"Found parent node in cache: {parent_id}")
        else:
            node = self._api.node.get(parent_id)
            if node:
                self.log.debug(f"Parent node found: {parent_id} and added to cache")
                self._nodecache[parent_id] = node
